...
do_set(DataFeed1_, 10); // implies scale with 20
do_set(DataFeed0_, 1); // implies 1st branch is taken, (transfer DAI + EUR)
...
assertEquals(Erc20_DAI.balanceOf(me).toNumber(), 19900, "Settlement asset unaffected by failed 2nd burn");
assertEquals(Erc20_DAI.balanceOf(contract.address).toNumber(), 100, "DC Settlement unaffected by failed 2nd burn");
...
assertEquals(Erc20_DAI.balanceOf(me).toNumber(), 20000, "Settlement asset is higher after 2nd (full) burn");
assertEquals(Erc20_DAI.balanceOf(contract.address).toNumber(), 0, "DC Settlement is lower after 2nd (full) burn");
...
assertEquals(Erc20_DAI.balanceOf(me).toNumber(), 20000, "Settlement asset unaffected by failed zero-burn");
